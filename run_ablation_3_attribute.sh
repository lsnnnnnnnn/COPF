...
run_one() {
  ...
  echo ">>> [Attribute Ablation] Running: ${exp_name}"
  mkdir -p "${outdir}"
  python -u scripts/run_copf.py --out_dir "${outdir}" "${COMMON_ARGS[@]}" "$@" --seed "${SEEDS}" |& tee "${outdir}/run.log"
}

# === (vii) Protected Attribute ===
# 1. Random (Node Mod)
run_one "group_nodemod" \
  --tgb_group_mode node_mod --tgb_group_n 5

# 2. Structural (Node Degree)
run_one "group_nodedegree" \
  --tgb_group_mode node_degree --tgb_group_n 5